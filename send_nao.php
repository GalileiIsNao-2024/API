...
# $connessione->query("SELECT price FROM products WHERE name = '$productName'")->fetch_assoc()["price"];
...
switch($req){
    ...
    case 'addToCart':
        ...
        $sql = "SELECT cart FROM users WHERE name = '$user'";
        if($result = $connessione->query($sql)) {
            if($result->num_rows == 1) {
    ...
    case 'checkout':
        ...
        if($result = $connessione->query($sql)) {
            if($result->num_rows == 1) {
                ...
                if($chronology = $connessione->query("SELECT chronology FROM users WHERE name = '$user'") and json_encode($cart) != '[]'){
